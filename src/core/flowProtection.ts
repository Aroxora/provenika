...
export class FlowProtection extends EventEmitter {
  ...
  constructor(config: FlowProtectionConfig = {}) {
    super();
    // Optional environment variable to disable protections for testing
    const bypassProtections = process.env['AGI_BYPASS_PROTECTIONS'] === '1';
    this.config = {
      detectInjection: config.detectInjection ?? !bypassProtections,
      injectionThreshold: config.injectionThreshold ?? (bypassProtections ? 100 : 70),
      protectFlow: config.protectFlow ?? true,
      protectUI: config.protectUI ?? true,
      rateLimitPerSecond: config.rateLimitPerSecond ?? 100,
      maxMessageLength: config.maxMessageLength ?? 1000000,
