...
import { installSignalHandlers, authorizedShutdown } from '../core/shutdown.js';
    ...
    for await (const event of controller.send(trimmed)) {
      if (event.type === 'message.start') {
        ...
        if (toolName === 'Bash' && args?.['command']) {
          stdout.write(`Running: $ ${args['command']}\n`);
        ...
        if (event.result && typeof event.result === 'string' && event.result.length > 0) {
          ...
          const prefix = isBash ? 'Done: ' : '';
          if (lines.length > 10) {
            stdout.write(prefix + lines.slice(0, 10).join('\n') + '\n... (' + (lines.length - 10) + ' more lines)\n');
          } else if (event.result.length > 300) {
            stdout.write(prefix + event.result.slice(0, 300) + '...\n');
          } else {
            stdout.write(prefix + event.result + '\n');
          }
        } else if (isBash) {
          stdout.write('Done:\n');
        }
