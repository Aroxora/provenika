...
// Stub types for removed tournament/security functionality
interface SecurityTournamentConfig { workingDir: string; providers?: string[]; projectIds?: string[]; autoFix?: boolean; includeZeroDay?: boolean; maxRounds?: number; onProgress?: (event: any) => void; }
interface TournamentOutcome { winner: any; rankings: any[]; ranked?: any[]; tie: boolean; }
interface TournamentCandidate { id: string; variant: string; policyId?: string; patchSummary?: string; metrics?: any; signals?: any; evaluatorScores?: any[]; rawOutput?: string; }
const runSecurityTournament = async (_config: SecurityTournamentConfig) => ({ summary: { totalRounds: 0, primaryWins: 0, refinerWins: 0, ties: 0, winningStrategy: '', totalFindings: 0, verifiedFindings: 0, fixedFindings: 0, criticalCount: 0, highCount: 0, mediumCount: 0, duration: 0 }, findings: [], remediation: { fixed: 0, failed: 0, skipped: 0 } });
const runDefaultSecurityAudit = async () => ({ findings: [], summary: { critical: 0, high: 0, medium: 0 } });
const runDualTournament = (_task: any, _candidates: any[], _options?: any): TournamentOutcome => ({ winner: null, rankings: [], ranked: [], tie: true });

// Stub type for UpgradeStepOutcome - now always an object type since we removed tournament code
interface UpgradeStepOutcomeObject {
  primary?: { summary?: string; score?: number; durationMs?: number; success?: boolean; tournament?: { aggregateScore?: number; humanAccuracy?: number; }; humanAccuracy?: number; };
  refiner?: { summary?: string; score?: number; durationMs?: number; success?: boolean; tournament?: { aggregateScore?: number; humanAccuracy?: number; }; humanAccuracy?: number; };
  winnerVariant?: string;
  winner?: { summary?: string; score?: number; tournament?: { aggregateScore?: number; }; };
  intent?: string;
}
...
import { runRepoUpgradeFlow, type RepoUpgradeFlowOptions, type EnhancedRepoUpgradeReport } from '../orchestration/index.js';

// Stub types (orchestrator has been removed)
type RepoUpgradeMode = 'single-continuous' | 'dual-rl-continuous' | 'dual-rl-tournament';
type RepoUpgradeReport = EnhancedRepoUpgradeReport & {
  mode?: RepoUpgradeMode;
  continueOnFailure?: boolean;
  objective?: string;
  repoPolicy?: string;
  variantWorkspaceRoots?: Partial<Record<'primary' | 'refiner', string>>;
  modules?: Array<{ status: string; label: string; steps: UpgradeStepOutcomeObject[]; validations?: Array<{ command: string; success: boolean; skipped?: boolean }>; }>;
  recommendations?: string[];
  variantStats?: { primaryWins: number; refinerWins: number; ties: number };
};
type UpgradeStepOutcome = UpgradeStepOutcomeObject;
type UpgradeVariant = 'primary' | 'refiner';
...
class InteractiveShell {
  ...
  private readonly pinnedPromptPath: string;
   ...
   */
  private savePinnedPrompt(text: string): void {
    try {
   ...
   */
  private clearPinnedPrompt(): void {
    try {
      if (existsSync(this.pinnedPromptPath)) {
  ...
  private async runRepoUpgradeCommand(args: string[]): Promise<void> {
    ...
    try {
      ...
      const createVariantController = async (variant: UpgradeVariant, workspaceRoot: string) => {
      ...
      this.renderUpgradeReport(report as any);
