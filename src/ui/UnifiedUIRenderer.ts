...
export function formatSecureToolsSummary(options?: {
  ...
  lines.push(`  ${chalk.gray('with enhanced output formatting.')}`);
  lines.push(`  ${chalk.gray('(src/capabilities/unifiedCodingCapability.ts)')}`);
...
export class UnifiedUIRenderer extends EventEmitter {
  ...
  private pasteRegions: Array<{
    start: number;           // Start index in buffer
    end: number;             // End index in buffer (exclusive)
    lineCount: number;       // Number of lines in the paste
    charCount: number;       // Total character count
    preview: string;         // First line preview (truncated)
  ...
  /** Pinned prompt - always visible at top of UI */
  private pinnedPrompt: {
    text: string;
    timestamp: number;
  } | null = null;
  ...
  private detectPasteLanguage(text: string): string | undefined {
  ...
  private readonly pasteCondenseMinChars = 200;
  private readonly pasteCondenseMinLines = 4;

  /** Record a paste region for condensed display */
  private recordPasteRegion(insertPos: number, content: string): void {
  ...
  private clearPasteRegions(): void {
    this.pasteRegions = [];
  }

  /** Adjust paste regions when characters are deleted from the buffer */
  private adjustPasteRegionsForDeletion(deleteStart: number, deleteEnd: number): void {
  ...
  private adjustPasteRegionsForInsertion(insertPos: number, insertLen: number): void {
  ...
  // Track recent stream/response content for cross-type deduplication
  private recentStreamResponseContent: Set<string> = new Set();
  ...
  constructor(
    ...
    this.boundResizeHandler = () => {
      if (!this.plainMode && !this.disposed) {
        try {
          this.updateTerminalSize();
          // Force a full re-render on resize to prevent display corruption
          this.forceNextRender = true;
          // Clear the last overlay to ensure complete redraw
  ...
  private insertText(text: string): void {
    ...
    // Insert text directly (preserving newlines for multi-line support)
  ...
  addEvent(type: RendererEventType, content: string): void {
    ...
    if (normalized !== 'prompt') {
      // Use content-only deduplication for stream/response to catch cross-type duplicates
      if (normalized === 'stream' || normalized === 'response') {
        const contentKey = content.trim();
        if (contentKey && this.recentStreamResponseContent.has(contentKey)) {
  ...
  private async renderEvent(event: UIEvent): Promise<void> {
    ...
    // Use content-only deduplication for stream/response to catch cross-type duplicates
    // (e.g., same content arriving as 'stream' then 'response')
    if (event.type === 'stream' || event.type === 'response') {
      const contentKey = event.content.trim();
      if (contentKey && this.recentStreamResponseContent.has(contentKey)) {
        return; // Skip duplicate content
      }
      if (contentKey) {
        this.recentStreamResponseContent.add(contentKey);
        // Limit set size to prevent memory growth - keep last 50 entries
        if (this.recentStreamResponseContent.size > 50) {
          const first = this.recentStreamResponseContent.values().next().value;
          if (first) this.recentStreamResponseContent.delete(first);
        }
      }
    }
  ...
  private sanitizeToolResultContent(content: string): string {
    ...
    for (const line of lines) {
      ...
      // Strip ANSI codes for pattern matching but preserve original line
      const strippedLine = this.stripAnsi(trimmed);

      // Skip empty lines at start
      if (cleanLines.length === 0 && !trimmed) continue;

      // ALWAYS preserve lines with important status indicators
      const hasImportantMarker = /\b(FAILED|ERROR|SUCCESS|ABORT|WARNING|PASS|FAIL|OK)\b/i.test(strippedLine);
      ...
      if (/^[*_]{2,}$/.test(strippedLine)) continue; // Just ** or __
      if (/^[|│┃]+$/.test(strippedLine)) continue; // Just vertical bars
      if (/^[─━═]+$/.test(strippedLine)) continue; // Just horizontal lines (but not lines with text like ═══ FAILED ═══)
      if (/^\.{2,}$/.test(strippedLine)) continue; // Just dots
      if (/^[\s️]+$/.test(strippedLine)) continue; // Just whitespace/emoji modifiers
      if (/^[:\s]+$/.test(strippedLine)) continue; // Just colons/whitespace
      if (/^[,\s]+$/.test(strippedLine)) continue; // Just commas/whitespace

      // Skip markdown bold/italic fragments that leaked
      if (/^\*{1,2}[^*]*\*{0,2}$/.test(strippedLine) && strippedLine.length < 10 && !/\w{3,}/.test(strippedLine)) continue;

      // Skip lines that are just short fragments with high punctuation ratio
      const alphaInLine = (strippedLine.match(/[a-zA-Z0-9]/g) || []).length;
      if (strippedLine.length > 0 && strippedLine.length < 20 && alphaInLine / strippedLine.length < 0.3) continue;

      // Skip very short lines that are just punctuation or single chars (except numbers)
      // Use strippedLine to ignore ANSI codes when checking length
      if (strippedLine.length <= 2 && !/^\d+$/.test(strippedLine) && !/^[a-zA-Z]{2}$/.test(strippedLine)) {
        // Allow specific short patterns like "OK", "0", "1" etc.
        if (!/^(?:ok|no|yes|\d+)$/i.test(strippedLine)) continue;
      }
      ...
      if (/^```\s*$/.test(strippedLine)) continue;
  ...
  */
  ...
  private formatCompactToolResult(content: string): string {
    ...
    if (readMatch) {
      ...
      // Use visual length (without ANSI codes) for truncation decisions
      const visualLen = this.stripAnsi(firstLine).length;
      const maxLen = 120;

      if (visualLen > maxLen) {
        // Strip ANSI for truncation calculation, then re-apply colors if needed
        const strippedLine = this.stripAnsi(firstLine);
        // Look for sentence end within limit
        const sentenceEnd = strippedLine.slice(0, maxLen).search(/[.!?;]\s|[.!?;]$/);
        if (sentenceEnd > 30) {
          summary = strippedLine.slice(0, sentenceEnd + 1);
        } else {
          // No good sentence break, truncate at word boundary
          const truncated = strippedLine.slice(0, maxLen);
  ...
  private colorResultSummary(summary: string): string {
    ...
    // Don't double-color if summary already contains ANSI codes
    if (summary.includes('\x1b[')) {
      return summary;
    }

    const lower = summary.toLowerCase();
    if (lower.includes('fail') || lower.includes('error') || lower.includes('abort')) {
      return theme.error(summary);
    }
    if (summary.startsWith('✓') || lower.includes('updated') || lower.includes('created') || lower.includes('written') || lower.includes('success')) {
  ...
  private formatEditResultWithDiff(content: string): string {
    ...
    if (content.match(/^⏺\s*(Update|Create)\s*\(/)) {
      ...
      const diffLines = lines.filter(line =>
        line.includes('\x1b[1;32m') || // green (additions)
  ...
  /**
   * Set a pinned prompt that stays visible at the top of the UI.
   * The pinned prompt persists across interactions until replaced or cleared.
   */
  setPinnedPrompt(text: string | null): void {
    ...
    if (this.pinnedPrompt?.text === trimmed) {
      return; // No change
    }
    this.pinnedPrompt = {
      text: trimmed,
  ...
  /**
   * Get the current pinned prompt text, or null if none.
   */
  getPinnedPrompt(): string | null {
    return this.pinnedPrompt?.text ?? null;
  }
  ...
  clearPinnedPrompt(): void {
    if (!this.pinnedPrompt) return;
    this.pinnedPrompt = null;
    this.renderPrompt();
  }
  ...
  private renderPromptImmediate(): void {
    ...
    const currentWidth = this.output.isTTY ? (this.output.columns || 80) : 80;
    if (!this.forceNextRender &&
        ...
        this.lastOverlay !== null) {
      // Content unchanged and terminal size unchanged, skip expensive re-render
      return;
    }
    ...
    const burstActive = this.pasteBurstWindowStart > 0 && now - this.pasteBurstWindowStart <= this.plainPasteWindowMs;
    const inEmitPaste = this.emitPasteBuffer.length > 0 || this.emitPasteTimer !== null;
    if (burstActive || this.inPlainPaste || this.inBracketedPaste || inEmitPaste) {
      return;
    }
  ...
  private buildCondensedDisplayBuffer(): { displayBuffer: string; cursorDisplayPos: number } {
  ...
  private expandCollapsedPasteToBuffer(): void {
    // Function kept for compatibility - paste blocks are no longer collapsed
  }
